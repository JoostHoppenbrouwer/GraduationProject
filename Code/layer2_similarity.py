...
# Loop over fooling images layer 2 feat
...
#for dir in ['n01530575']:
    ...
    layer2_fooling = np.load(subdir + '/layer2_output.npy')
    ...
    for classdir in next(os.walk(data_root))[1]:
        ...
        for directory in next(os.walk(classfolder))[1]:
            ...
            # Load and setup
            layer2_output = np.load(os.path.join(classfolder, directory) + '/layer2_output.npy')
    ...
    plt.savefig(folder + "/layer2_similarity.jpg", bbox_inches='tight')
    pickle.dump( final_sims, open( folder + "layer2_sims.p", "wb" ) )
