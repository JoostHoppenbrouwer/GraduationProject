...
# Loop over fooling images layer 5 feat
...
#for dir in ['n01530575']:
    ...
    layer5_fooling = np.load(subdir + '/layer5_output.npy')
    ...
    for classdir in next(os.walk(data_root))[1]:
        ...
        for directory in next(os.walk(classfolder))[1]:
            ...
            # Load and setup
            layer5_output = np.load(os.path.join(classfolder, directory) + '/layer5_output.npy')
    ...
    plt.savefig(folder + "/layer5_similarity.jpg", bbox_inches='tight')
    pickle.dump( final_sims, open( folder + "layer5_sims.p", "wb" ) )
