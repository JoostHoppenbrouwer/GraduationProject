...
# Loop over fooling images layer 2 after pooling and normalization feat
...
#for dir in ['n01530575']:
    ...
    layer2_fooling = np.load(subdir + '/layer2_pooling_norm.npy')
    ...
    plt.title('Layer 2 pooling norm similarity')
    ...
    for classdir in next(os.walk(data_root))[1]:
        ...
        for directory in next(os.walk(classfolder))[1]:
            ...
            # Load and setup
            layer2_output = np.load(os.path.join(classfolder, directory) + '/layer2_pooling_norm.npy')
    ...
    plt.savefig(folder + "/layer2_pooling_norm_similarity.jpg", bbox_inches='tight')
    pickle.dump( final_sims, open( folder + "layer2_pooling_norm_sims.p", "wb" ) )
