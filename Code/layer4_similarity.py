...
# Loop over fooling images layer 4 feat
...
#for dir in ['n01530575']:
    ...
    layer4_fooling = np.load(subdir + '/layer4_output.npy')
    ...
    for classdir in next(os.walk(data_root))[1]:
        ...
        for directory in next(os.walk(classfolder))[1]:
            ...
            # Load and setup
            layer4_output = np.load(os.path.join(classfolder, directory) + '/layer5_pooling.npy')
            ...
            for j in range(NUM_KERNELS):
                x = (j / 20) * 14
                y = (j % 20) * 14
                fooling_data = layer4_fooling[x : x + KERNEL_SIZE, y : y + KERNEL_SIZE]
                class_data = layer4_output[x: x + KERNEL_SIZE, y : y + KERNEL_SIZE]
    ...
    plt.savefig(folder + "/layer4_similarity.jpg", bbox_inches='tight')
    pickle.dump( final_sims, open( folder + "layer4_sims.p", "wb" ) )
