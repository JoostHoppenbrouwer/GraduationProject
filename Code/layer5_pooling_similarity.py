...
# Loop over fooling images layer 5 after pooling feat
...
#for dir in ['n01530575']:
    ...
    layer5_fooling = np.load(subdir + '/layer5_pooling.npy')
    ...
    plt.title('Layer 5 pooling similarity')
    ...
    for classdir in next(os.walk(data_root))[1]:
        ...
        for directory in next(os.walk(classfolder))[1]:
            ...
            # Load and setup
            layer5_output = np.load(os.path.join(classfolder, directory) + '/layer5_pooling.npy')
            ...
            for j in range(NUM_KERNELS):
                x = (j / 16) * 7
                y = (j % 16) * 7
                fooling_data = layer5_fooling[x : x + KERNEL_SIZE, y : y + KERNEL_SIZE]
                class_data = layer5_output[x: x + KERNEL_SIZE, y : y + KERNEL_SIZE]
    ...
    plt.savefig(folder + "/layer5_pooling_similarity.jpg", bbox_inches='tight')
    pickle.dump( final_sims, open( folder + "layer5_pooling_sims.p", "wb" ) )
