...
means_root = '../Data/fooling_images/means/layer6/'
stdevs_root = '../Data/fooling_images/stdevs/layer6/'

# Loop over fooling images layer 6 feat
for dir in next(os.walk(data_root))[1]:
    ...
    layer6_output = np.load(subdir + '/layer6_activation.npy')

    plt.figure(figsize=(20,10))
    plt.figure(1)
    plt.title('Layer 6 accuracy for several thresholds (o = fooling class, x = other class)')
    ...
    plt.figure(2)
    plt.title('Layer 6 cosine similarity (o = fooling class, x = other class)')
    plt.xlim(0,2)
    ...
    plt.xlim(0,2)
    plt.xticks( np.arange(2) )
    plt.title('Layer 6 euclidean distance (o = fooling class, x = other class)')
    ...
    f = open(folder + "layer6_distance.txt", "w")
    ...
    for THRESHOLD in [0.0, 0.1, 0.25, 0.5, 1]:
        ...
        for file in next(os.walk(means_root))[2]:
            ...
            layer6_output1 = layer6_output[index_array]
            ...
            if THRESHOLD == 0.0:
                ...
                # Cosine similarity
                plt.figure(2)
                cos_sim = 1-(dst.cosine(class_mean1, layer6_output1))
                ...
                # Euclidean distance
                plt.figure(3)
                euc_dst = dst.euclidean(class_mean1, layer6_output1)
                if (name == dir):
        ...
        if THRESHOLD == 0.0:
            pickle.dump( cos_sims, open( folder + "layer6_cos_sims.p", "wb" ) )

    f.close()
    plt.figure(1)
    plt.savefig(subdir + "/distance/layer6_accuracy.jpg", bbox_inches='tight')
    plt.close()
    plt.figure(2)
    plt.savefig(subdir + "/distance/layer6_cosine_similarity.jpg", bbox_inches='tight')
    plt.close()
    plt.figure(3)
    plt.savefig(subdir + "/distance/layer6_euclidean_distance.jpg", bbox_inches='tight')
    plt.close()
    plt.close("all")    
