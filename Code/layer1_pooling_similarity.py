...
# Loop over fooling images layer 1 after pooling feat
...
#for dir in ['n01530575']:
    ...
    layer1_fooling = np.load(subdir + '/layer1_pooling.npy')
    ...
    plt.title('Layer 1 pooling similarity')
    ...
    for classdir in next(os.walk(data_root))[1]:
        ...
        for directory in next(os.walk(classfolder))[1]:
            ...
            # Load and setup
            layer1_output = np.load(os.path.join(classfolder, directory) + '/layer1_pooling.npy')
    ...
    plt.savefig(folder + "/layer1_pooling_similarity.jpg", bbox_inches='tight')
    pickle.dump( final_sims, open( folder + "layer1_pooling_sims.p", "wb" ) )
